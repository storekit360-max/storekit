                    ...
                    return (
                      <article key={product._id} className="product-card group">
                        <Link to={`/product/${product.slug}`} className="product-card-media block relative overflow-hidden bg-gray-50" style={{aspectRatio:'1/1'}}>
                          <img src={product.thumbnail||product.images?.[0]||'https://via.placeholder.com/300'} alt={product.name} width="600" height="600" loading={page===1?'eager':'lazy'} decoding="async" fetchPriority={page===1?'high':'auto'} sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 25vw" className="card-img w-full h-full object-cover"/>
