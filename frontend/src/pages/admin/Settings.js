...
const GATEWAY_PRESETS = {
  ...
  koko: { name: 'KOKO', logo: '🟢', color: '#168a52', fields: [{ key:'merchantId', label:'Merchant ID', type:'text' }, { key:'apiKey', label:'API Key', type:'password' }, { key:'publicKey', label:'Public Key', type:'textarea' }, { key:'privateKey', label:'Private Key', type:'textarea' }, { key:'endpoint', label:'KOKO API Endpoint URL', type:'url' }, { key:'logoUrl', label:'KOKO Logo URL', type:'url' }] },
};
                      ...
                      <div className="space-y-3">
                        {(Array.isArray(settings.businessWelcomeStores)?settings.businessWelcomeStores:[]).map((store,index)=><div key={index} className="rounded-xl bg-gray-50 border border-gray-200 p-4">
                          ...
                          <div className="grid sm:grid-cols-2 gap-3">
                            ...
                            <F label="Destination URL" value={store.url||''} onChange={e=>setSettings(p=>({...p,businessWelcomeStores:p.businessWelcomeStores.map((s,i)=>i===index?{...s,url:e.target.value}:s)}))} placeholder="/store or https://other-store.com/store"/>
                  ...
                  <div className="rounded-2xl overflow-hidden shadow-xl border border-gray-100">
                    ...
                    <div className="px-5 py-4 flex items-center justify-between" style={{background:settings.darkBgColor||currentTheme.dark}}>
                      <div className="flex items-center gap-2.5">
                        {settings.logoUrl ? <img src={settings.logoUrl} alt="" style={{height:`${settings.logoSize||56}px`,maxWidth:'180px',objectFit:'contain'}}/> : <span className="font-bold text-white text-base" style={{fontFamily:currentFont.display}}>{settings.storeName||'StoreKit'}</span>}
                      </div>
                  ...
                  {Object.entries(FONTS).map(([key, font]) => (
                    <button key={key} onClick={()=>applyFont(key)}
                      ...
                      <div className="flex items-start justify-between gap-2 mb-3">
                        <p className="text-xl font-bold text-gray-900 leading-tight" style={{fontFamily:font.display}}>{settings.storeName||'StoreKit'}</p>
