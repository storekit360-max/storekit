...
const SECTION_DEFS = {
  homepage: [
    ...
    { id: 'bestsellers',     label: '🏷️ Flash Deals',          desc: 'Discounted products row', icon: '🏷️' },
    { id: 'best_selling',    label: '🏆 Best Selling Products', desc: 'Hot-selling and fast-moving products', icon: '🏆' },
...
export default function LayoutEditor() {
  ...
  const [savedSections, setSavedSections] = useState(() => {
    try { return JSON.parse(localStorage.getItem('storekit_saved_sections') || '[]'); } catch { return []; }
  });
  ...
  const saveAs = () => {
    ...
    localStorage.setItem('storekit_saved_sections', JSON.stringify(updated));
    setSaveAsName('');
    setShowSaveAs(false);
