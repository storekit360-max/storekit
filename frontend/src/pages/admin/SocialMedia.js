...
function PostManagementTab({ connectedPlatforms }) {
  ...
  const [scheduleForm,setScheduleForm] = useState(()=>{const start=new Date(Date.now()+10*60000-new Date().getTimezoneOffset()*60000);const defaults={name:`Product posts ${start.toISOString().slice(0,10)}`,startAt:start.toISOString().slice(0,16),gapMinutes:5,productsPerDay:5,offerPercent:0,voucherCode:'',includeSinhala:true,ctaType:'shop_now',customDescription:''};try{return {...defaults,...JSON.parse(localStorage.getItem('shopzen.socialSchedule.form')||'{}')}}catch{return defaults}});

  const loadSchedules=useCallback(async()=>{
    const loadAllQueueItems=async()=>{
      const first=await API.get('/social-scheduling/queue',{params:{limit:250,page:1}});
      const items=[...(first.data.items||[])];
      const pages=Math.max(1,Number(first.data.pages)||1);
      for(let page=2;page<=pages;page++){
        const response=await API.get('/social-scheduling/queue',{params:{limit:250,page}});
        items.push(...(response.data.items||[]));
      }
      return items;
    };
    const [jobs,batches,drafts]=await Promise.allSettled([loadAllQueueItems(),API.get('/social-scheduling/draft-batches',{params:{limit:50}}),API.get('/social-scheduling/draft-batches')]);
  ...
  const createPostSchedule=async()=>{
    ...
    setScheduling(true);
    try{
      const {data}=await API.post('/social-scheduling/draft-batches',{
        selection:{productIds:[...selectedIds]},
        config:{platforms:[...selPlatforms],name:scheduleForm.name,startDate:scheduleForm.startAt.slice(0,10),dailyStartTime:scheduleForm.startAt.slice(11,16),postGapMinutes:Number(scheduleForm.gapMinutes),productsPerDay:Number(scheduleForm.productsPerDay),
          additionalDiscountPercent:Number(scheduleForm.offerPercent)||0,voucherCode:scheduleForm.voucherCode,includeSinhala:scheduleForm.includeSinhala,cta:scheduleForm.ctaType},
      },{timeout:180000});
      toast.success(`${data.count || 0} product descriptions generated for review. Nothing has been scheduled yet.`);
      const dynamicDescription=(draft)=>{const product=products.find(item=>String(item._id)===String(draft.product))||{};const price=product.price||'';const salePrice=product.salePrice||product.discountPrice||price;const discount=price&&salePrice&&Number(price)>Number(salePrice)?`${Math.round((1-(Number(salePrice)/Number(price)))*100)}%`:'';const values={'{{productName}}':product.name||draft.productName||'','{{price}}':price,'{{salePrice}}':salePrice,'{{discount}}':discount,'{{brand}}':product.brand||'','{{sku}}':product.sku||'','{{category}}':product.category?.name||product.category||'','{{description}}':product.shortDescription||product.description||'','{{url}}':product.slug?`${window.location.origin}/product/${product.slug}`:'','{{delivery}}':'Islandwide Delivery'};return Object.entries(values).reduce((text,[token,value])=>text.split(token).join(String(value||'')),scheduleForm.customDescription||'')};
      openScheduleDraft({ _id:data.draftGroup, platforms:[...selPlatforms], ctaType:scheduleForm.ctaType, items:(data.drafts || []).map(draft => ({...draft, productId:String(draft.product), caption:[draft.caption || draft.generatedContent || '', dynamicDescription(draft).trim()].filter(Boolean).join('\n\n'), scheduledAt:draft.scheduledFor, productName:draft.productName || 'Product'})), startAt:scheduleForm.startAt });
      await loadSchedules();
    }catch(error){toast.error(error.response?.data?.message||'Preview generation failed');}
    finally{setScheduling(false)}
  };

  const saveDraft=async()=>{if(!reviewDraft)return;setDraftSaving(true);try{setReviewDraft(current=>({...current,items:draftItems}));toast.success('Description edits saved for this review session');}finally{setDraftSaving(false)}};
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-violet-100 p-5 space-y-4">
        ...
        <div className="text-xs text-gray-500 block"><label className="font-semibold text-gray-700">Fully customizable post description</label><textarea className="form-input mt-2 w-full min-h-[260px] sm:min-h-[340px] resize-y text-sm leading-6" maxLength={5000} placeholder="Write your campaign message here. Drag dynamic fields below into any position…" value={scheduleForm.customDescription} onChange={e=>setScheduleForm({...scheduleForm,customDescription:e.target.value})} onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();const value=e.dataTransfer.getData('text/plain');if(value)setScheduleForm(form=>({...form,customDescription:`${form.customDescription}${form.customDescription?'\n\n':''}${value}`}))}}/><div className="mt-3 rounded-xl border border-blue-100 bg-blue-50/50 p-3"><p className="text-[11px] font-semibold text-blue-800 mb-2">Drag dynamic data into your description</p><div className="flex flex-wrap gap-2">{[['{{productName}}','Product name'],['{{price}}','Regular price'],['{{salePrice}}','Sale price'],['{{discount}}','Discount %'],['{{brand}}','Brand'],['{{sku}}','SKU'],['{{category}}','Category'],['{{description}}','Product description'],['{{url}}','Product link'],['{{delivery}}','Delivery text']].map(([token,label])=><button type="button" draggable onDragStart={e=>e.dataTransfer.setData('text/plain',token)} key={token} className="px-2.5 py-1.5 rounded-lg bg-white text-blue-700 text-[11px] border border-blue-200 cursor-grab active:cursor-grabbing hover:bg-blue-100">↕ {label}</button>)}</div><p className="text-[11px] text-blue-600 mt-2">Example: Dynamic fields are replaced with each product data.</p></div><span className="block text-[11px] text-gray-400 mt-2">Your template is applied to every selected product, then each product’s dynamic values are filled automatically. The source product data is never changed.</span></div>
        {selectedIds.size>0&&<div className="rounded-xl border border-gray-200 bg-gray-50 p-3"><p className="text-xs font-semibold text-gray-600 mb-2">Database descriptions (read-only)</p><div className="space-y-2 max-h-40 overflow-y-auto">{products.filter(product=>selectedIds.has(product._id)).map(product=><div key={product._id} className="rounded-lg bg-white border border-gray-100 p-2"><p className="text-xs font-medium text-gray-700">{product.name}</p><p className="text-xs text-gray-500 mt-1 whitespace-pre-wrap">{product.shortDescription||product.description||'No description stored for this product.'}</p></div>)}</div><p className="text-[11px] text-gray-400 mt-2">These descriptions come from the product database and cannot be changed here. Use the custom description above to add your campaign message.</p></div>}
