...
export default function InstallmentQuote({ productId, className = '' }) {
  ...
  return (
    <div className={`installment-quote mt-2 w-full min-w-0 overflow-hidden ${cardMode ? 'card-installments text-sm' : 'rounded-xl bg-violet-50 px-2.5 py-2 text-xs'} ${className}`}>
      {!cardMode && <div className="font-semibold text-violet-700">Installment available</div>}
      {state.loading ? <div className="text-violet-400">Installment available</div> : state.plans.slice(0, 3).map(plan => {
        ...
        return (
          <div key={`${provider}-${plan.months}`} className={`mt-1 flex min-w-0 flex-wrap items-center gap-x-1.5 gap-y-0.5 leading-5 ${cardMode ? 'text-gray-500' : 'text-violet-600'}`}>
